...
class JobScraper:
    ...
    async def _ensure_position(self, company_id: str, position_title: str) -> str:
        ...
        result = self.supabase.table("positions").select("id").eq("company_id", company_id).eq("title", position_title).execute()
        
        if result.data:
            return result.data[0]["id"]
        
        # Create new position
        result = self.supabase.table("positions").insert({
    ...
    async def _get_existing_phone_hashes(self, position_id: str) -> List[str]:
        ...
        result = self.supabase.table("candidates").select("phone_sha256").eq("position_id", position_id).execute()
        ...
        async with httpx.AsyncClient() as client:
            ...
            while True:
                ...
                if status_data["status"] == "finished":
                    ...
                    raise Exception(f"Browser-Use task failed: {status_data.get('error', 'Unknown error')}")
                
                # Continue polling if status is "running" or "pending"
    
    async def _store_candidates(self, position_id: str, candidates_data: List[Dict[str, str]], source_url: str) -> Dict[str, int]:
